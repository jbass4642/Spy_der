...
DISCORD_WEBHOOK_URLS = [
    "https://discord.com/api/webhooks/1340918653369057310/AyRrXAIrVspe1rncslVZTrjeTVGKh82khCmkQNn39tdLLJa8fm6DBpNWzjP2utzs0M5C",
    "https://discord.com/api/webhooks/1420576936907903108/8p58XmtTR97AGBIV_MbUHN6kIdVvA5n86RrqK86RQxpeRUojI3lfNF89bra3S8q2vCb_",
    "https://discord.com/api/webhooks/1340970279433928745/9tyBBB0nsCxLgBpReAKDGN21-aZIihgvRUfzKFUNC-FpaFIyGah1OgLMJI0n2SRIYRGQ",
    "https://discord.com/api/webhooks/1341225435102445599/Hr7YXB4dsGlbhWj8ZjYXVpqO19NnCTBIuJedM4VJ0-hCBIUtDuN0VD5P4HW2pQiroucA",
    "https://discord.com/api/webhooks/1420581830335135877/29AIYiKjz1TZM2gKf4eiUSN09hZBar4nJqId9pN15B1fXkP8C4XxTYCm8bW4ziZj47ZF",
    "https://discord.com/api/webhooks/1420944975922729051/oyFadaiOKvUYkZu2HoHHHNdouwK7p-k15Mjn-efS97vnxuiR6uPIdwHHV02xezjuhC6E"
]
...
def find_vix_drop_times(spy_price_data, vix_price_data, put_wall):
    """Identify timestamps where SPY price is at/near Put Wall and VIX is dropping."""
    ...
    merged_data = spy_price_data[['datetime', 'close']].merge(
        vix_price_data[['datetime', 'open', 'close']],
        on='datetime',
        how='inner',
        suffixes=('_spy', '_vix')
    )
    ...
    for i in range(1, len(merged_data)):
        spy_close = merged_data['close_spy'].iloc[i]
        ...
        # Check conditions: SPY at/near Put Wall and VIX dropping
        is_spy_near_put_wall = spy_close <= put_wall + 0.25
        is_vix_dropping = vix_close < vix_prev_close or vix_open < vix_close
        ...
        if is_spy_near_put_wall and is_vix_dropping:
            ...
            logger.info(f"VIX drop condition met at {naive_timestamp}: SPY close={spy_close:.2f}, Put Wall={put_wall:.2f}, VIX close={vix_close:.2f}, VIX prev close={vix_prev_close:.2f}")
...
def fetch_options_chain(ticker, expiry_date=None):
    ...
    try:
        ...
        calls = calls.groupby('strike').agg({'volume': 'sum', 'oi': 'sum', 'delta': 'mean', 'vanna': 'mean', 'charm': 'mean'}).reset_index()
        puts = puts.groupby('strike').agg({'volume': 'sum', 'oi': 'sum', 'delta': 'mean', 'vanna': 'mean', 'charm': 'mean'}).reset_index()
...
def get_nearest_strikes(calls, puts, S, num_strikes=17):
    ...
    combined = combined.merge(calls, on='strike', how='left').rename(columns={'volume': 'call_volume', 'oi': 'call_oi', 'delta': 'call_delta', 'vanna': 'call_vanna', 'charm': 'call_charm'})
    combined = combined.merge(puts, on='strike', how='left').rename(columns={'volume': 'put_volume', 'oi': 'put_oi', 'delta': 'put_delta', 'vanna': 'put_vanna', 'charm': 'put_charm'})
    combined = combined.fillna({'call_volume': 0, 'put_volume': 0, 'call_oi': 0, 'put_oi': 0, 'call_delta': 0, 'put_delta': 0, 'call_vanna': 0, 'put_vanna': 0, 'call_charm': 0, 'put_charm': 0})
    ...
    logger.info(f"Selected {len(combined)} strikes, Volume: Put Wall: {put_wall}, Call Wall: {call_wall}, POC: {poc} | OI: Put Wall: {put_wall_oi}, Call Wall: {call_wall_oi}, POC: {poc_oi}")
...
def create_net_volume_chart(combined, S):
    ...
    fig.update_layout(
        title=dict(text="Net Put-Call Volume", font=dict(size=18, color='white')),
...
def create_net_oi_chart(combined, S):
    ...
    fig.update_layout(
        title=dict(text="Net Put-Call Open Interest", font=dict(size=18, color='white')),
        xaxis_title=dict(text="Strike", font=dict(size=14, color='white')),
...
def create_spy_candlestick_chart(spy_price_data, put_wall, call_wall, poc, vix_price_data):
    ...
    for idx, drop_time in enumerate(drop_times):
        fig.add_vline(
            x=drop_time,
            line_dash="dashdot",
            line_color="orange",
    ...
    fig.update_layout(
        title=dict(text="1-Minute SPY Heikin-Ashi Candlestick", font=dict(size=18, color='white')),
...
def create_vix_candlestick_chart(price_data):
    ...
    fig.update_layout(
        title=dict(text="1-Minute VIX Heikin-Ashi Candlestick", font=dict(size=18, color='white')),
        xaxis_title=dict(text="Time", font=dict(size=14, color='white')),
...
def create_rsi_chart(price_data):
    ...
    fig.update_layout(
        title=dict(text="1-Minute SPY RSI (8-period)", font=dict(size=18, color='white')),
        xaxis_title=dict(text="Time", font=dict(size=14, color='white')),
        yaxis_title=dict(text="RSI", font=dict(size=14, color='white')),
        height=300,
        plot_bgcolor='black',
        paper_bgcolor='black',
        font=dict(color='white', size=12),
...
def create_volume_table(combined):
    ...
    table_html += '<tr><th>Strike</th><th>Call Volume</th><th>Put Volume</th><th>Net Vol (Puts - Calls)</th><th>Call OI</th><th>Put OI</th><th>Net OI (Puts - Calls)</th><th>Net Delta-Weighted Vol</th><th>Net Delta-Weighted OI</th><th>Net Vanna Vol</th><th>Net Vanna OI</th><th>Net Charm Vol</th><th>Net Charm OI</th></tr>'
...
def capture_combined_screenshot(combined, S, spy_price_data, vix_price_data, put_wall, call_wall, poc):
    try:
        ...
        vanna_fig = create_net_vanna_chart(combined, S)
        vanna_oi_fig = create_net_vanna_oi_chart(combined, S)
        charm_fig = create_net_charm_chart(combined, S)
        ...
        vanna_img = pio.to_image(vanna_fig, format='png', width=800, height=300)
        vanna_oi_img = pio.to_image(vanna_oi_fig, format='png', width=800, height=300)
        charm_img = pio.to_image(charm_fig, format='png', width=800, height=300)
        ...
        images = [
            ...
            Image.open(io.BytesIO(vanna_img)),
            Image.open(io.BytesIO(vanna_oi_img)),
            Image.open(io.BytesIO(charm_img)),
            Image.open(io.BytesIO(charm_oi_img))
        ]
        if spy_candle_fig:
            spy_candle_img = pio.to_image(spy_candle_fig, format='png', width=800, height=500)
            images.append(Image.open(io.BytesIO(spy_candle_img)))
        if vix_candle_fig:
            vix_candle_img = pio.to_image(vix_candle_fig, format='png', width=800, height=500)
            images.append(Image.open(io.BytesIO(vix_candle_img)))
        if rsi_fig:
            rsi_img = pio.to_image(rsi_fig, format='png', width=800, height=300)
            images.append(Image.open(io.BytesIO(rsi_img)))
...
def send_discord_alert():
    ...
    try:
        ticker = 'SPY'
        calls, puts, S = fetch_options_chain(ticker)
        ...
        screenshot = capture_combined_screenshot(combined, S, spy_price_data, vix_price_data, put_wall, call_wall, poc)
        if not screenshot:
        ...
        if rsi_available:
            rsi = calculate_rsi(spy_price_data['close'], period=8)
            if len(rsi) >= 2:
                rsi_slope = rsi.iloc[-1] - rsi.iloc[-2]
                if S <= put_wall and rsi_slope > 0 and time.time() - last_alert_time >= 300:  # 5 minutes
                    special_payload = {
                        'content': f"**SPY Rebound Alert**\nPrice at or below Put Wall: ${S:.2f}\nRSI Slope Positive: {rsi_slope:.2f}\nEntry Price: ${S:.2f}\nExit Price (POC): ${poc:.2f}"
                    }
                    ...
                    for webhook_url in DISCORD_WEBHOOK_URLS:
                        try:
                            ...
                            if special_response.status_code == 204:
                                logger.info(f"Special Discord alert sent successfully to {webhook_url}")
                            else:
...
<head>
    ...
    <style>
        ...
        #chart, #oi-chart, #delta-weighted-chart, #delta-weighted-oi-chart, #vanna-chart, #vanna-oi-chart, #charm-chart, #charm-oi-chart { width: 100%; height: 400px; }
        ...
        function updateDashboard() {
            fetch('/data')
                ...
                .then(data => {
                    ...
                    Plotly.newPlot('vanna-chart', JSON.parse(data.vanna_chart).data, JSON.parse(data.vanna_chart).layout);
                    Plotly.newPlot('vanna-oi-chart', JSON.parse(data.vanna_oi_chart).data, JSON.parse(data.vanna_oi_chart).layout);
                    Plotly.newPlot('charm-chart', JSON.parse(data.charm_chart).data, JSON.parse(data.charm_chart).layout);
...
def get_data():
    ...
    vanna_chart = create_net_vanna_chart(combined, S).to_json()
    vanna_oi_chart = create_net_vanna_oi_chart(combined, S).to_json()
    charm_chart = create_net_charm_chart(combined, S).to_json()
...
scheduler.add_job(send_discord_alert, 'interval', minutes=5, next_run_time=datetime.now(CDT))
scheduler.start()
